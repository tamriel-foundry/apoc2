...
class Apoc_Shortcodes {
	...
	static function spoiler( $atts, $content = NULL ) {
		...
		$thespoiler .= '<p class="spoiler-title double-border bottom"><i class="fa fa-warning"></i> <strong>Spoiler:</strong> <em>' . esc_attr($title) . '</em></p>';
	...
	static function quote( $atts, $content = NULL ) {
		...
		// Otherwise, make it a formatted or nested discussion quote
			...
			// Format the quote
			$thequote = '<div class="quote">';
			$thequote .= '<p class="quote-author double-border bottom"><i class="fa fa-quote-left"></i> <strong>'.$author_slug.'</strong> said on '.$quote_link.' :</p>';
