...
<div id="top-login" class="<?php echo $logged_in ? 'logged-in' : 'logged-out'; ?>">
	...
	<form name="top-login-form" id="top-login-form" action="<?php echo SITEURL . '/wp-login.php'; ?>" method="post">			
		...
		<fieldset>	
			...
			<a class="admin-bar-button button" href="<?php echo trailingslashit(SITEURL) . 'register'; ?>" title="Register a new user account!"><i class="fa fa-user"></i>Register</a>
			<a class="admin-bar-button button" href="<?php echo wp_lostpassword_url(); ?>" title="Lost your password?"><i class="fa fa-question"></i>Lost Password</a>			
		</fieldset>
		<?php endif; ?>
